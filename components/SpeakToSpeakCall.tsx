...
export const SpeakToSpeakCall: React.FC<SpeakToSpeakCallProps> = ({
  ...
  const equalizerRef = useRef<HTMLDivElement | null>(null);
  ...
  const cleanupLive2D = () => {
    if (live2DModelRef.current) {
      ...
      if ((modelInstance as any)._mouseMoveHandler) {
        window.removeEventListener('pointermove', (modelInstance as any)._mouseMoveHandler);
      }
      try {
        modelInstance.destroy(true);
      } catch (e) {}
    ...
    if (live2DAppRef.current) {
      try {
        live2DAppRef.current.destroy(true, { children: true });
      } catch (e) {}
  ...
  useEffect(() => {
    ...
    if (!PIXI || !PIXI.live2d) {
      console.warn("PixiJS or Live2D library not found in window");
      return;
    }
    ...
    const initLive2D = async () => {
      try {
        ...
        live2DModelRef.current = loadedModel;
        ...
        const handleMouseMove = (e: PointerEvent) => {
          if (!loadedModel || !canvasEl) return;
          const rect = canvasEl.getBoundingClientRect();
          const x = e.clientX - rect.left;
          const y = e.clientY - rect.top;
          loadedModel.focus(x, y);
        };

        window.addEventListener('pointermove', handleMouseMove);
        (loadedModel as any)._mouseMoveHandler = handleMouseMove;

        loadedModel.on('pointertap', () => {
          const motionGroups = ['Tap', 'Tap@Body', 'Flick', 'FlickUp', 'FlickDown'];
          const randomGroup = motionGroups[Math.floor(Math.random() * motionGroups.length)];
          loadedModel.motion(randomGroup);
        });

        const analyserBuffer = new Uint8Array(64);
        
        pixiApp.ticker.add(() => {
          if (!loadedModel || !loadedModel.internalModel || !loadedModel.internalModel.coreModel) return;
          const core = loadedModel.internalModel.coreModel;
          
          let mouthOpen = 0;
          
          if (isActiveRef.current && isModelSpeakingRef.current) {
            if (outAnalyserNodeRef.current) {
              outAnalyserNodeRef.current.getByteTimeDomainData(analyserBuffer);
              let sum = 0;
              for (let i = 0; i < analyserBuffer.length; i++) {
                const deviation = Math.abs(analyserBuffer[i] - 128);
                sum += deviation;
              }
              const average = sum / analyserBuffer.length;
              mouthOpen = Math.min(1.0, average / 11);
            } else {
              mouthOpen = Math.sin(Date.now() / 80) * 0.35 + 0.35 + (Math.random() * 0.3);
              mouthOpen = Math.max(0, Math.min(1.0, mouthOpen));
            }
          }
    ...
    const timer = setTimeout(initLive2D, 150);
  ...
  useEffect(() => {
    if (!isActive || !micStream) {
      if (discRef.current) discRef.current.style.transform = 'scale(1) rotate(0deg)';
      if (atmosphereRef.current) atmosphereRef.current.style.transform = 'scale(1.25)';
      if (pingRingRef.current) {
        pingRingRef.current.style.width = '290px';
        pingRingRef.current.style.height = '290px';
      }
      if (dbTextRef.current) dbTextRef.current.innerText = '0 dB';
      if (equalizerRef.current) {
        const children = equalizerRef.current.children;
        for (let i = 0; i < 32; i++) {
          const bar = children[i + 1] as HTMLDivElement;
          if (bar) {
            bar.style.height = '8%';
    ...
    try {
      ...
      const animate = (timestamp: number) => {
        ...
        if (pingRingRef.current) {
          const ringSize = Math.min(290 * targetScale, 420);
          pingRingRef.current.style.width = `${ringSize}px`;
          pingRingRef.current.style.height = `${ringSize}px`;
        }
        if (dbTextRef.current) {
          dbTextRef.current.innerText = `${Math.round(volume)} dB`;
        }

        // Direct DOM update of 32 frequency bars (no React state updates)
        if (equalizerRef.current) {
          const children = equalizerRef.current.children;
          ...
          for (let i = 0; i < 32; i++) {
            ...
            const val = Math.max(6, Math.min(100, (rawVal / 255) * 85 + b));
            const bar = children[i + 1] as HTMLDivElement;
            if (bar) {
              bar.style.height = `${isActive ? Math.max(8, val) : 8}%`;
              bar.style.boxShadow = (val > 30 && isActive) ? `0 0 8px ${currentTheme.badge}` : 'none';
            }
          }
        }
              ...
              <div className="relative w-[310px] h-[390px] flex flex-col items-center justify-between pointer-events-auto rounded-3xl border border-white/10 bg-[#0e1015]/60 shadow-[0_20px_50px_rgba(0,0,0,0.6)] backdrop-blur-xl p-4 transition-all hover:border-[#8ab4f8]/30">
                ...
                {isActive && (
                  <div
                    ...
                    style={{
                      borderColor: `${currentTheme.badge}30`,
                ...
                {/* Call AI / End Call Button inside capsule flow */}
                <div className="relative z-30 flex items-center justify-center pt-2 pb-1 w-full shrink-0">
                  {!isActive ? (
                    <button
                      onClick={onStartCall}
                      disabled={isConnecting}
                      className="bg-white hover:bg-gray-50 text-gray-900 border-none rounded-full py-3 px-5.5 flex items-center gap-2.5 cursor-pointer text-sm font-bold shadow-[0_10px_25px_rgba(0,0,0,0.3)] hover:scale-105 active:scale-95 transition-all"
                      title="Click to start Voice to Voice call"
                    >
                      <div
                        className="w-7 h-7 rounded-full text-white flex items-center justify-center text-xs shadow-md"
                        style={{ background: currentTheme.btnBg }}
                      >
                    ...
                    <button
                      ...
                      className="bg-red-600 hover:bg-red-700 text-white border-none rounded-full py-3 px-5.5 flex items-center gap-2.5 cursor-pointer text-sm font-bold shadow-[0_10px_25px_rgba(239,68,68,0.4)] hover:scale-105 active:scale-95 transition-all"
                      title="Disconnect live call"
                    >
                      <div className="w-7 h-7 rounded-full bg-white text-red-600 flex items-center justify-center text-xs shadow-md">
              ...
              <>
                <div
                  ...
                  style={{
                    ...
                    maxWidth: '240px',
                    maxHeight: '240px',
                    boxShadow: `0 20px 50px ${currentTheme.glowColor}, 0 0 30px ${currentTheme.glowColor}`,
                ...
                {/* Overlaid Center Call Action Button (For non-Priya disc mode) */}
                <div className="absolute z-30 flex items-center justify-center">
                  {!isActive ? (
                    ...
                    <button
                      ...
                      title="Disconnect live call"
                    >
                      <div className="w-8 h-8 rounded-full bg-white text-red-600 flex items-center justify-center text-xs shadow-md">
