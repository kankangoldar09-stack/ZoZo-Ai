...
import { ZoZoLogo } from "./components/ZoZoLogo";
...
const App: React.FC = () => {
  ...
  const [outAnalyserNode, setOutAnalyserNode] = useState<AnalyserNode | null>(null);
  ...
  useEffect(() => {
    if (!auth) {
      console.warn("Firebase Auth is not initialized. Running in local fallback mode.");
      setIsAuthLoading(false);
      return;
    }

    // Safety fallback: if auth state doesn't resolve in 4 seconds, force loading to false
    const authTimeout = setTimeout(() => {
      console.warn("Auth initialization timed out, forcing isAuthLoading to false");
      setIsAuthLoading(false);
    }, 4000);
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      if (u && db) {
        try {
          ...
          console.error("Firestore initialization failed for user:", dbErr);
        }
      }
    }, (error) => {
      clearTimeout(authTimeout);
  ...
  const handleGoogleLogin = async () => {
    if (!auth) {
      setAuthError("Firebase Auth is currently unavailable. Running in local sandbox.");
      return;
    }
    try {
      setAuthError("");
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    setAuthError("");
    if (!auth) {
      setAuthError("Firebase Auth is currently unavailable.");
      return;
    }
    try {
      if (authMode === "login") {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
          throw new Error("Microphone access is blocked because this page is not running in a secure context (HTTPS or localhost). Please access the app via http://localhost:3000 or configure HTTPS.");
        }
        ...
        try {
          if (navigator.permissions && navigator.permissions.query) {
            ...
            if (permissionStatus.state === 'denied') {
              throw new Error("Microphone permission has been blocked by your browser! Please click the site settings icon (lock/microphone icon) in the browser address bar next to the URL, change 'Block' to 'Allow' for Microphone, and reload the page.");
            }
          }
        } catch (e: any) {
      ...
      try {
        ...
        throw new Error("Microphone request failed: " + (micErr.message || micErr) + ". Please check your browser settings and grant microphone access.");
      }
      ...
      if (!apiKey || apiKey.trim() === "") {
        throw new Error("Gemini API Key is missing! Please save your Gemini API Key in the Plugins modal (click the gear icon or Plugins option) before starting the call.");
      }
      ...
      if (outAudioCtxRef.current && !outAnalyserRef.current) {
        const analyser = outAudioCtxRef.current.createAnalyser();
        analyser.fftSize = 128;
        analyser.smoothingTimeConstant = 0.75;
        analyser.connect(outAudioCtxRef.current.destination);
        outAnalyserRef.current = analyser;
        setOutAnalyserNode(analyser);
      }
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onmessage: async (msg: LiveServerMessage) => {
            ...
            // 1. Process user input transcription (save to chat history)
            ...
            // 2. Process model output transcription (save to chat history)
            ...
            // 3. Decode and stream raw audio data natively in the selected voice
            const audioData =
            ...
            if (audioData && outAudioCtxRef.current && isVoiceEnabledRef.current) {
              ...
              if (outAnalyserRef.current) {
                s.connect(outAnalyserRef.current);
              } else {
...
8. **Smart & Accurate**: Real human vibe ke sath-sath coding, facts aur research mein 100% sharp raho!`,
      ...
      alert(`Voice Call Error: ${e.message || e}`);
      stopSession();
    }
  };
